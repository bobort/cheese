...
from accounts.managers import StudentManager, OrderLineItemQuerySet, AvailableProductsManager
from utils.models import HTMLMixin
...
class Student(HTMLMixin, AbstractUser):
    MD, DO = range(0, 2)
    DEGREE_CHOICES = (
        (MD, "MD"),
        (DO, "DO"),
    ...
    name = models.CharField(max_length=182, blank=True)
    ...
    def get_full_name(self):
        return f"{self.name}, {self.get_degree_display()}" if self.degree is not None else self.name

    def get_absolute_url(self):
        return reverse('accounts:view', kwargs={'pk': self.pk})
...
class Order(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('accounts:receipt', kwargs={'pk': self.pk})
    ...
    def save(self, *args, **kwargs):
        ...
        if self.student:
            self.student.balance_paid = False
            self.student.save()
